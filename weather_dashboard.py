...
#预测图
forecast_data=df['temperature_2m_min (°C)']
print("预测数据:", forecast_data.head())
# ADF 检验
...
if result[1] > 0.05:
    ...
    print('序列平稳')
seasonal_diff = forecast_data.diff(365).dropna()
# 再做一次普通差分（如果序列还不平稳）
#easonal_diff = forecast_data.diff().dropna()
...
auto_model = auto_arima(
    ...
    m=7,                  # 季节周期（天），年周期
    trace=True,             # 打印搜索过程
    stepwise=True,          # 逐步搜索（快）
    suppress_warnings=True,
    error_action='ignore',
    n_jobs=-1,              # 并行加速
    information_criterion='aic'  # 用 AIC 选择模型
)

print("最佳模型阶数 (p,d,q) =", auto_model.order)
print("最佳季节阶数 (P,D,Q,m) =", auto_model.seasonal_order)
from statsmodels.stats.diagnostic import acorr_ljungbox
from statsmodels.graphics.tsaplots import plot_acf

# 假设 auto_model 是 pmdarima 拟合的模型对象
# 获取内部 statsmodels 结果对象
res = auto_model.arima_res_
...
# 假设 forecast_data 是原始温度序列（日期索引），auto_model 已拟合
# 取最近30天历史数据
history = forecast_data.tail(30)
...
forecast = fitted_final.forecast(steps=7, exog=X_future)
conf_int = fitted_final.get_forecast(steps=7, exog=X_future).conf_int()
...
fig_humidity.add_trace(go.Scatter(x=future_dates, y=forecast, mode='lines+markers', name='预测湿度', line=dict(dash='dash')))
fig_humidity.add_trace(go.Scatter(x=future_dates, y=conf_int.iloc[:,1], mode='lines', line=dict(width=0), showlegend=False))
fig_humidity.add_trace(go.Scatter(x=future_dates, y=conf_int.iloc[:,0], mode='lines', name='95%置信区间', fill='tonexty', fillcolor='rgba(255,192,203,0.5)'))
...
figs = {
    ...
    'weather_forecast':('🔮未来天气预测',fig_forecast),
    'humidity_forecast':('🌬️未来湿度预测',fig_humidity)
